...
With that said, I personally find Google SRE Guide Troubleshooting section quite comprehensive[^1] and references the hypothetico-deductive model (which is no less than the scientific method) as proposed path. In essence, observing, creating an hypothesis, creating predictions based on that hypothesis, and put them to test. Another great approach for performance analysis in particular is the [USE method](https://www.brendangregg.com/usemethod.html) proposed by Brendan Gregg, which suggests creating a checklist of checkpoints (just like the one a pilot would use before take off/landing) that can display utilization, saturation and errors of resources commonly affected. This method is pragmatic and can yield results very fast without getting too much into the analaysis. There are many other sources for methodologies. There are even techniques exclusive to root cause analysis like the simple _5 whys_, _RPR_, and other covered by _Problem Management_ domain in _ITILv3_ literature.

Most would agree that a thorough observation and data gathering has to take place in the begging. Which is what I am going to focus on in the following section.
