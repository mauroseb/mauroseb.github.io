...
A ton has been written on the matter and arguably many methods and techniques may compete to be proven effective under specific circumstances. But firstly it has to be acknowledged that the complexity of modern software systems makes the process not precisely deterministic, and that at the time there is critical impact the expert may choose the tools that her/his experience is dicating in order to address the problem in the fastest manner. With that said, I personally find Google SRE Guide Troubleshooting section quite comprehensive[5] and references the hypothetico-deductive model as proposed method, but there are many other sources for methodologys too. There are techniques exclusive to root cause analysis like the simple _5 whys_, _RPR_, and other covered by _Problem Management_ domain in _ITILv3_ literature.
